#!/Users/itai.ganot/miniconda3/envs/py27/bin/python
# This script is based on Steve Gargan's script but matched to my needs.
# It allows grouping by machine's ostype and/or group membership.
#
...
class ConsulInventory(object):
    ...
    def __init__(self):
        ...
        self.inmemory_kv = []
        self.inmemory_nodes = []
        self.nodes_by_groups = {}
        self.nodes_by_ostype = {}
    ...
    def load_data_for_node(self, node, datacenter):
        ...
        if 'ostype' in node['Meta']:
            self.add_metadata(node_data, "consul_ostype", node['Meta']['ostype'])
        if 'groups' in node['Meta']:
            self.add_metadata(node_data, "consul_groups", node['Meta']['groups'])

        self.load_groups_from_node_groups(node)
        self.load_ostype_from_node_meta(node)
    ...
    def load_groups_from_node_groups(self, node):
        if 'groups' in node['Meta']:
            groups = node['Meta']['groups']
            for group in groups.split(','):
                self.add_node_to_map(self.nodes_by_groups, group.strip(), node)

    def load_ostype_from_node_meta(self, node):
        if 'ostype' in node['Meta']:
            ostypes = node['Meta']['ostype']
            for ostype in ostypes.split(','):
                self.add_node_to_map(self.nodes_by_ostype, ostype.strip(), node)
    ...
    def combine_all_results(self):
        ...
        groupings = [self.nodes, self.nodes_by_datacenter, self.nodes_by_service,
                     self.nodes_by_tag, self.nodes_by_kv, self.nodes_by_availability, self.nodes_by_groups, self.nodes_by_ostype]
...
class ConsulConfig(dict):
    ...
    def read_settings(self):
        ...
        config_options = ['host', 'token', 'datacenter',
    ...
    def read_cli_args(self):
        ...
        parser.add_argument('--groups', action='store',
                            help='get a list of specific group')
